...
The foundation migration installs pgvector and creates only `campaign`. P7-01 adds preparation artifacts/versions, generation runs, generic generated assets, role links, lifecycle audit, and immutability triggers. P1-01 adds logical Library documents/path history, exact immutable revisions/chunks, lexical vectors, ingestion runs, and scope-safe corpus snapshots in `0003_library_sources`. P2 adds versioned embedding profiles/derivations (`0004`), resumable snapshot-pinned embedding runs (`0005`), and immutable source-body-free retrieval audit records (`0006`). Alembic's own `alembic_version` table is the schema-version record; a second application schema-version table would duplicate that authority.
