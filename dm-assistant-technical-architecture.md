...
Prompted dungeon attempts use the terminal stage taxonomy `model_transport`, `model_submission`, `intent_compile`, `deterministic_preflight`, `render`, `asset_stage`, `persistence`, and `completed`. Each terminal outcome records a stable public-safe code and body-free diagnostics; a model-transport failure may additionally retain only its allowlisted normalized transport category, while unexpected failures retain only correlation, stage, and exception class in ordinary logs. A prompt-attempt run starts before provider contact. It is distinct from, and links to, the final artifact generation run so a failed pre-package attempt is durable without implying an artifact exists.
...
The model-visible schema and runtime contract must use the provider-supported JSON Schema subset and stay small enough to inspect. The model supplies room identity/purpose and progression primitives—critical path, branches, bounded loops, secrets, gates, and content intent—not an arbitrary complete edge list. Code constructs the graph. Every non-null room encounter reserves a separate later authoring task, so structural guidance requires only the requested content-slot kinds and counts and leaves unrelated encounter fields null; it does not populate every room merely because the schema permits it. Room-local content uses at most one nonempty record per room, with optional objective values null or omitted rather than empty outside the one named objective room. A schema-invalid submission may receive one bounded repair that preserves the complete original structural instruction and retains only body-free diagnostics, duration, and measured usage; deterministic topology or geometry bugs never trigger a request for another random model graph.

At the narrow Node transport boundary, the pinned `@earendil-works/pi-ai` Codex adapter accepts generic `maxTokens` but correctly omits a model-output field from its ChatGPT subscription request. The former gateway overlay of `max_output_tokens` was rejected before output by two authorized canaries; an opt-in no-store fingerprint confirmed that the provider error named that field as a rejected parameter without retaining provider text/body in logs or the attempt report. Provider-free review of official OpenAI Codex source at `bc39b0e`, open enhancement [openai/codex#36180](https://github.com/openai/codex/issues/36180), pinned `pi-ai` 0.84.1, and registry-latest 0.84.4 found no supported `max_output_tokens`, `max_tokens`, or `max_completion_tokens` alternative. The gateway therefore removed the overlay and advertises no `hard_output_token_limit` capability for `openai-codex`. Synthetic OAuth/injected-fetch coverage captures and decompresses the outbound SSE body and proves all three unsupported fields are absent. By contrast, OpenAI's separate public API OpenAPI `CreateResponse` contract supports `max_output_tokens`; synthetic API-key/injected-fetch coverage proves the standard `openai` adapter serializes the exact requested value, and that provider advertises `hard_output_token_limit`. Codex subscription remains usable under the provider-aware alpha policy: strict measured post-response and cumulative ceilings, repair reservation, timeout/cancellation, and fail-before-publication behavior remain mandatory, while quota consumed before an over-limit response cannot be recovered.
