...
At the narrow Node transport boundary, the pinned `@earendil-works/pi-ai` Codex adapter's generic `maxTokens` option is not sufficient because that adapter omits the Responses hard-limit field. The gateway therefore overlays the exact Workbench-requested `max_output_tokens` value through the adapter's pre-dispatch payload callback. The transform runs after payload construction and before either SSE or WebSocket dispatch, overrides any stale adapter value, and fails closed if the adapter payload is not an object. A provider-free transport test captures and decompresses the actual SSE request using only synthetic OAuth and an injected fetch; no prompt/provider body is persisted. Measured post-response usage validation remains mandatory defense in depth but is not treated as provider-side enforcement.
