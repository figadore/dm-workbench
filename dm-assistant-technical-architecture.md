...
Before preparation approval, a final continuity/readiness gate recomputes projection-hash and authorized-source inheritance, exact dependencies, required content, lineage, secrecy, and typed cross-task consistency. Whole-dungeon qualities that cannot be proven mechanically are evaluated separately: a bounded read-only holistic reviewer may emit structured diagnostics for thematic reinforcement across rooms, history-to-environment causality, mechanic/objective unity, progression, intentional motif variation, and consistency with selected campaign lore. It cannot edit the artifact, clear a deterministic blocker, approve preparation, or write canon. The DM-facing review packet records ratings and explicit disposition of findings; accepted fixes use targeted regeneration of one exact enrichment seam rather than a silent whole-guide rewrite. No model can approve the result or make it canon.
