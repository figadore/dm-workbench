...
def test_one_step_coordinator_publishes_one_puzzle_child_then_plans_exploration(
    ...
    specification = DungeonStudioSpecification.model_validate(parent.specification)
    puzzle_room_id = next(
        room.id for room in specification.package.rooms if room.role.value == "puzzle"
    )
    gateway = FakeGatewayClient(
    ...
    child_specification = DungeonStudioSpecification.model_validate(child.specification)
