"""Durable attempt lifecycle for independently bounded feature interaction enrichment."""
...
class DungeonFeatureInteractionPromptAttemptResult:
    attempt_run_id: uuid.UUID
    result: DungeonWorkflowResult | None
    public_code: str


class DungeonFeatureInteractionPromptApplicationService:
    """Persist minimal task metadata while accepted content stays with the artifact."""

    def __init__(
        self,
        preparation: PreparationService,
        prompts: DungeonFeatureInteractionPromptService,
    ) -> None:
        ...
        *,
        ...
        with bind_log_context(attempt_run_id=str(attempt_id)):
            try:
                ...
                logger.warning(
                    "dungeon feature interaction prompt attempt stopped",
                    extra={
                        "event_data": {
                ...
                return DungeonFeatureInteractionPromptAttemptResult(
                    attempt_id, None, code
                )

            code = (
                "dungeon_feature_interaction_prompt_completed"
                if result.success
                else "dungeon_feature_interaction_prompt_failed"
            )
            ...
            logger.info(
                "dungeon feature interaction prompt attempt finished",
                extra={
                    "event_data": {
                        "stage": report["stage"],
                        "code": code,
        ...
        *,
        ...
        return self._preparation.start_generation_run(
            StartGenerationRun(
                ...
                generation_kind="dungeon_feature_interaction_prompt",
                input_scope={
                    "task_type": "dungeon_feature_interaction_enrichment",
                    "surface": surface,
...
def _failure_report(error: Exception) -> tuple[str, dict[str, JsonValue]]:
    if isinstance(error, DungeonFeatureInteractionRejectedAfterRepair):
        code = "dungeon_feature_interaction_prompt_rejected_after_repair"
        return code, {
...
def _diagnostic_codes(error: Exception) -> list[str]:
    if not isinstance(error, DungeonFeatureInteractionRejectedAfterRepair):
        return []
    return [
        code
