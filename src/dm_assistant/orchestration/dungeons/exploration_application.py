"""Durable attempt lifecycle for independently bounded exploration enrichment."""
        ...
        *,
        ...
        with bind_log_context(attempt_run_id=str(attempt_id)):
            try:
                ...
                logger.warning(
                    "dungeon exploration prompt attempt stopped",
                    extra={
                        "event_data": {
                ...
                return DungeonExplorationPromptAttemptResult(attempt_id, None, code)

            code = (
                "dungeon_exploration_prompt_completed"
                if result.success
                else "dungeon_exploration_prompt_failed"
            )
            ...
            logger.info(
                "dungeon exploration prompt attempt finished",
                extra={
                    "event_data": {
                        "stage": report["stage"],
                        "code": code,
            ...
            return DungeonExplorationPromptAttemptResult(attempt_id, result, code)
        ...
        *,
        ...
        return self._preparation.start_generation_run(
            StartGenerationRun(
                ...
                generation_kind="dungeon_exploration_prompt",
                input_scope={
                    "task_type": "dungeon_exploration_enrichment",
                    "surface": surface,
...
def _failure_report(error: Exception) -> tuple[str, dict[str, JsonValue]]:
    if isinstance(error, DungeonExplorationRejectedAfterRepair):
        code = "dungeon_exploration_prompt_rejected_after_repair"
        return code, {
    ...
    if isinstance(error, StructuredSubmissionBudgetExceeded):
        code = "dungeon_exploration_prompt_token_budget_exhausted"
        return code, {
    ...
    if isinstance(error, StructuredSubmissionRepairBudgetExhausted):
        code = "dungeon_exploration_prompt_repair_budget_exhausted"
        return code, {
            "stage": "model_submission",
            "code": code,
            **error.report(),
        }
    code = "dungeon_exploration_prompt_failed"
...
def _diagnostic_codes(error: Exception) -> list[str]:
    if not isinstance(error, DungeonExplorationRejectedAfterRepair):
        return []
    return [
        code
