...
_PLAN_GUIDANCE = (
    ...
    "not repeat an encounter kind across rooms unless the request asks for multiples. Map "
    "every requested exploration challenge to one rooms[].encounter value of 'exploration', "
    "make that count exactly match the request, and place it in the requested room or branch. "
    "Give each such room one room_contents[].feature that supplies a local environmental "
    "affordance for the later exploration task. The encounter value reserves that later task; "
    "it is not the challenge prose. Keep structural descriptions brief. "
