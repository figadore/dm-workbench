"""Durable attempt lifecycle for independently bounded room narratives."""
        ...
        *,
        ...
        with bind_log_context(attempt_run_id=str(attempt_id)):
            try:
                ...
                logger.warning(
                    "dungeon room narrative prompt attempt stopped",
                    extra={
                        "event_data": {
                ...
                return DungeonRoomNarrativePromptAttemptResult(attempt_id, None, code)

            code = (
                "dungeon_room_narrative_prompt_completed"
                if result.success
                else "dungeon_room_narrative_prompt_failed"
            )
            ...
            logger.info(
                "dungeon room narrative prompt attempt finished",
                extra={
                    "event_data": {
                        "stage": report["stage"],
                        "code": code,
            ...
            return DungeonRoomNarrativePromptAttemptResult(attempt_id, result, code)
        ...
        *,
        ...
        return self._preparation.start_generation_run(
            StartGenerationRun(
                ...
                generation_kind="dungeon_room_narrative_prompt",
                input_scope={
                    "task_type": "dungeon_room_narrative_enrichment",
                    "surface": surface,
...
def _failure_report(error: Exception) -> tuple[str, dict[str, JsonValue]]:
    if isinstance(error, DungeonRoomNarrativeRejectedAfterRepair):
        code = "dungeon_room_narrative_prompt_rejected_after_repair"
        return code, {
    ...
    if isinstance(error, StructuredSubmissionBudgetExceeded):
        code = "dungeon_room_narrative_prompt_token_budget_exhausted"
        return code, {
    ...
    if isinstance(error, StructuredSubmissionRepairBudgetExhausted):
        code = "dungeon_room_narrative_prompt_repair_budget_exhausted"
        return code, {
            "stage": "model_submission",
            "code": code,
            **error.report(),
...
def _diagnostic_codes(error: Exception) -> list[str]:
    if not isinstance(error, DungeonRoomNarrativeRejectedAfterRepair):
        return []
    return [
        code
