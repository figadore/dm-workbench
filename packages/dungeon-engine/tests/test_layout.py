...
def test_corridors_and_doors_are_orthogonal(layout_request: LayoutRequest) -> None:
    ...
    for door in result.package.composable_doors:
        assert (
...
def test_invalid_topology_fails_without_partial_package(
    ...
    assert layout_request.mechanics_plan is not None
    broken_plan = layout_request.mechanics_plan.model_copy(
        update={"connection_ids": (), "door_mechanics": ()}
    )
    result = generate_layout(
        layout_request.model_copy(
            update={"topology": broken_topology, "mechanics_plan": broken_plan}
        )
    )
