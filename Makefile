...
.PHONY: bootstrap dev-db dev-api dev-gateway stack-up stack-down stack-logs stack-smoke stack-token import-campaign-sources import-rules-sources dungeon-review-packet test-integration check
...
dungeon-review-packet:
	uv run --frozen python scripts/dungeon-guide-review-packet.py $(if $(REVIEW_OUTPUT),--output "$(REVIEW_OUTPUT)",)
